...
| `report-path`           | no       | `index.html`                      | Entry page to link to, relative to this branch's report directory. Use a path under `custom/` to select a custom report and skip reg-cli's HTML report generation. The selected file must exist under `.reg/`. |
...
`vrt-approval.yml` takes `r2-bucket`, `report-domain`, and `report-path` (same meanings as above) -- pass the same `report-domain` and `report-path` values given to `vrt-report`, if any.

For example, set `report-command` to `pnpm run vrt:generate-report` and `report-path` to `custom/index.html`. The command should put its HTML entry page and supporting files under `.reg/custom/`. That entry page becomes the PR comment and commit status link. The action still compares screenshots and writes `.reg/out.json`, but skips reg-cli's HTML report generation and HTML post-processing. The action fails if the selected entry page is missing.
